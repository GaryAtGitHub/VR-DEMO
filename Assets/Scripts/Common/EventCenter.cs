...
public class EventCenter
{
    ...
    private static void OnListenerAdding(EventDefine eventType, Delegate callBack)
    {
        ...
        {
            throw new Exception(string.Format("Trying to add a different delegate for event {0}. The the delegate type for current event is {1}. The added delegate type is {2}.", eventType, d.GetType(), callBack.GetType()));
        }
    }
